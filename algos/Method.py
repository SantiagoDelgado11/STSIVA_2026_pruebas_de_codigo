...
class Method:
    ...
    def sample(
        ...
        for i in pbar:
            ...
            for _ in range(2):
                x = self.H_t(model, i, x)
                x = self.F_t(forward_pass, transpose_pass, y, CG_iter, x, i)
            ...
            x = torch.sqrt(alpha_hat_prev) * x + torch.sqrt(1 - alpha_hat_prev) * noise

            difference = y - forward_pass(x)
            norm = torch.linalg.norm(difference)

            pbar.set_description(f"Sampling - Step {i} - Consistency: {norm.item():.4f}")
